...
struct routines_t routines[] =
{
    ...
    { "zgemm_batch", test_zgemm_batch },
    { "dgemm_batch", test_dgemm_batch },
    ...
    /* { "csyrk_batch", test_csyrk_batch }, */
    /* { "ssyrk_batch", test_ssyrk_batch }, */


    /* { "ztrmm_batch", test_ztrmm_batch }, */
    /* { "dtrmm_batch", test_dtrmm_batch }, */
...
/***************************************************************************//**
 *
 * @brief Tests and times a BBLAS routine.
...
void print_routine_usage(const char *name, param_value_t pval[])
{
    ...
    printf("\n"
           ...
           "(e.g., --gs=32,64) and may be repeated (--gs=32 --gs=64).\n"
           "Numeric options accept start:end:step ranges (--gs=32:128:32).\n"
           "\n");
...
void param_read(int argc, char **argv, param_t param[])
{
    ...
    for (int i = 2; i < argc && argv[i]; i++) {
        ...
        //--------------------------------------------------
        ...
        else if (param_starts_with(argv[i], "--ng="))
            err = param_scan_int(strchr(argv[i], '=')+1, &param[PARAM_NG]);
            ...
            err = param_scan_int(strchr(argv[i], '=')+1, &param[PARAM_INCG]);
    ...
    if (param[PARAM_INCG].num == 0)
        param_add_int(10, &param[PARAM_INCG]);
    ...
    if (param[PARAM_ALPHA].num == 0) {
        bblas_complex64_t z = 1.2345 + 2.3456*_Complex_I;
        param_add_complex(z, &param[PARAM_ALPHA]);
    }
    if (param[PARAM_BETA].num == 0) {
        bblas_complex64_t z = 6.7890 + 7.8901*_Complex_I;
...
int param_scan_complex(const char *str, param_t *param)
{
    ...
    do {
        ...
        bblas_complex64_t z = re + im*_Complex_I;
